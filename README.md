...
二、第二章之運用
1.串列型別(List Type)
		我們使用list去記錄使用者所選過的地點，並加預估的費用加總並顯示。
2.序對形別(Tuple Type)
		作為日曆資料庫對應用
三、第三章之運用
1.條件分支(if…elif…else…)
		用於語言系統的分流與景點介紹的選擇。
2.While迴圈
		做為多個地點選擇用，不會讓使用者只選擇一個地點後，程式就結束。
3.自訂函式(Def function)
		可以使重複的程式精簡化，在優化或增加功能時比較清楚直接。
4.內建函式(sum())
		加總列表中的元素，用以計算總金額。
 
參、Python Coding
一、函式庫匯入及資料宣告
#import calendar做日期規劃，以及利用list做預算估計
import calendar 
Estimated_amount=[]
二、使用input語法，讓使用者選擇中/英文版本：
print('台灣旅遊景點簡介\n')
#中英文
